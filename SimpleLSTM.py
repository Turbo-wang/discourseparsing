...
class SimpleLSTM:
    def __init__(self, max_len=50, hidden_layer=1280, num_classes=2, embedding_matrix, sequence_length):
        self.sequence_length = sequence_length
        self.hidden_layer = hidden_layer
        self.n_class = num_classes
        # Placeholders for input, output and dropout
        self.x1_indexs = tf.placeholder(tf.int32, [None, sequence_length], name="x1_indexs")
        self.x1_pos = tf.placeholder(tf.float32, [None, sequence_length, 46], name="x1_pos")

        self.x2_indexs = tf.placeholder(tf.int32, [None, sequence_length], name="x2_indexs")
        self.x2_pos = tf.placeholder(tf.float32, [None, sequence_length, 46], name="x2_pos")
    ...
    # def RNN(x, weights, biases):
        ...
        x1 = tf.transpose(self.embedded_chars_expanded1, [1, 0, 2])
        ...
        x2 = tf.transpose(self.embedded_chars_expanded2, [1, 0, 2])
        # Reshaping to (n_steps*batch_size, n_input)
